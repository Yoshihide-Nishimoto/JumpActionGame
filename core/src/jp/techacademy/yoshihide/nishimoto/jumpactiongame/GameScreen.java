...
public class GameScreen extends ScreenAdapter {
    ...
    public GameScreen(JumpActionGame game) {
        ...
        mPrefs = Gdx.app.getPreferences("jp.techacademy.yoshihide.nishimoto.jumpactiongame"); // ←追加する
        mHighScore = mPrefs.getInteger("HIGHSCORE", 0); // ←追加する

        death_sound = Gdx.audio.newSound(Gdx.files.internal("death.mp3"));
        star_sound = Gdx.audio.newSound(Gdx.files.internal("star.mp3"));
        jump_sound = Gdx.audio.newSound(Gdx.files.internal("jump.mp3"));
        fall_sound = Gdx.audio.newSound(Gdx.files.internal("fall.mp3"));
    ...
    private void createStage() {
        ...
        mEnemy.setPosition(WORLD_WIDTH / 2 - mEnemy.getWidth() / 2, 4.0f);
